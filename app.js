...
try {
	...
	console.log("Could not find local-vcap.json, skipping")
}
...
const run = async () => {
	...
	try {
		const putResults = await s3Client.send(new PutObjectCommand({
			Bucket: creds.bucket,
			Key: 'sample.txt',
			Body: 'Hello World!'
		}));
		console.log("Successfully created sample.txt");
	} catch (err) {
